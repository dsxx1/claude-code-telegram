    ...
    async def execute_command(
        ...
        try:
            ...
            base_prompt = (
                ...
                "2) выбрать способ доставки по расширению:\n"
                "   • .txt и .md → инструмент send_link_via_gdrive — он "
                "заливает файл на Google Drive и возвращает публичную "
                "share-ссылку. Эту ссылку процитируй пользователю одной "
                "строкой в ответе.\n"
                "   • любой другой формат (xlsx, csv, pdf, docx, json, "
                "log и т.д.) → инструмент send_file_to_user с абсолютным "
                "путём и кратким caption — придёт вложением в Telegram.\n"
                "НИКОГДА не вставляй содержимое файла код-блоком в чат "
                "вместо реальной отправки — пользователю нужен сам файл "
                "(вложение) или Drive-ссылка. Для картинок используй "
                "send_image_to_user."
                "\n\n## ДЕСТРУКТИВНЫЕ ОПЕРАЦИИ — БЕЗ ПОДТВЕРЖДЕНИЯ НЕ ВЫПОЛНЯТЬ\n"
                "Перед выполнением необратимых действий ты ОБЯЗАН СНАЧАЛА "
                "написать в чат, что собираешься сделать и почему, и "
                "явно спросить «Подтвердить?». Запускать команду только "
                "после ответа пользователя «да/ок/yes».\n"
                "К деструктивным относятся (не исчерпывающий список):\n"
                "• rm -rf, rm с маской, Remove-Item -Recurse -Force\n"
                "• git push --force, git reset --hard, git clean -fd, "
                "git branch -D, git checkout -- (на правках), git stash drop\n"
                "• DROP TABLE/DATABASE, TRUNCATE, DELETE без WHERE\n"
                "• dd, mkfs, format, fdisk\n"
                "• перезапись существующих файлов через Write без явной "
                "просьбы пользователя\n"
                "• массовые правки (>10 файлов) без явного согласия\n"
                "Если сомневаешься — лучше спроси, чем потеряй данные."
            )
